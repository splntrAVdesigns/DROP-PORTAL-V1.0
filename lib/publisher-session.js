...
export function sameOrigin(req){
  ...
  if(typeof origin!=='string'||!hosts.length)return false;
  try{
    const uri=new URL(origin);
    return uri.protocol==='https:'&&hosts.some(host=>uri.host===host);
  }catch{return false;}
}
export function validPublisherSession(req,key){
  if(!key||(req.method!=='GET'&&!sameOrigin(req)))return false;
