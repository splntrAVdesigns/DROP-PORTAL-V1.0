import {validInquiry,validSchedule,validDate,localDate,nextWeekly,cleanProfile} from './contracts.js';
...
export function effectiveProfile(schedule,inquiry){
  ...
  return {source:override?'weeklyOverride':'baseProfile',targetDropDate:date,...cleanProfile(override?inquiry.weeklyOverride:inquiry.baseProfile)};
}
...
export function createStage(candidate,{schedule,inquiry,manifest,archive},clock=new Date()){
  const captured=captureOccurrence(schedule,inquiry);
  assert(candidate?.captured?.nextDropAt===captured.nextDropAt &&
    candidate?.captured?.inquiryUpdatedAt===captured.inquiryUpdatedAt,
    'Research candidate uses a stale schedule or inquiry timestamp');
  assert(JSON.stringify(candidate?.profileSnapshot)===JSON.stringify(captured.profile),
    'Research candidate used different tuner weights');
