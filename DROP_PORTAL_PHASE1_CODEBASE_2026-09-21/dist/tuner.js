...
function capture(){
  ...
  const searchPast=dialog().querySelector('#search-past');if(searchPast)drafts[scope].searchPast=searchPast.value;
...
function adopt(result,keepDrafts=false){
  snapshot=result;
  if(!keepDrafts){drafts={base:{searchPast:'1mo',...result.inquiry.baseProfile},weekly:{searchPast:'1mo',...(result.inquiry.weeklyOverride||result.inquiry.baseProfile)}};dirty=false;}
...
function render(){
  ...
  <fieldset ${busy?'disabled':''} class="tuner-fields">${labels.map(([k,label])=>`<label class="range-label" for="tune-${k}">${label}<output id="out-${k}">${p[k]}</output></label><input class="range-input" id="tune-${k}" data-pref="${k}" type="range" min="${k==='count'?10:0}" max="${k==='count'?15:100}" value="${p[k]}">`).join('')}<label class="check"><input type="checkbox" id="mixes-enabled" ${p.mixes?'checked':''}>Include DJ mixes</label><label for="search-past">SEARCH THE PAST</label><select id="search-past">${[['1mo','1 month'],['3mo','3 months'],['6mo','6 months'],['1yr','1 year']].map(([value,label])=>`<option value="${value}" ${p.searchPast===value?'selected':''}>${label}</option>`).join('')}</select><p class="notice">Timeframe limits discovery by confirmed release date. Discovery depth changes candidate priority within that timeframe. Tracks with only an upload date need release-date review.</p><button class="primary tuner-save" data-tuner-save ${unconfigured?'disabled title="Publisher credentials required"':''}>${busy?'SAVING…':'Save '+(scope==='base'?'base profile':'this week’s override')}</button>${scope==='weekly'?`<button class="tuner-save" data-tuner-clear ${unconfigured?'disabled title="Publisher credentials required"':''}>Use base profile this week</button>`:''}
...
async function save(kind){
  ...
  try{
    ...
    else {snapshot=result;if(kind==='clear')drafts.weekly={searchPast:'1mo',...result.inquiry.baseProfile};scheduleDraft=pendingSchedule;}
    dirty=JSON.stringify(cleanProfile(drafts.base))!==JSON.stringify(cleanProfile(result.inquiry.baseProfile))||JSON.stringify(cleanProfile(drafts.weekly))!==JSON.stringify(cleanProfile(result.inquiry.weeklyOverride||result.inquiry.baseProfile));
...
export function setupTuner(options){
  onSaved=options.onSaved;notify=options.toast;setupPersonalTuner();
  ...
  dialog().addEventListener('input',e=>{if(e.target.matches('[data-pref],#mixes-enabled,#search-past')){dirty=true;const output=dialog().querySelector('#out-'+e.target.dataset.pref);if(output)output.textContent=e.target.value;const status=dialog().querySelector('.tuner-sync');if(status)status.textContent='UNSAVED EDITS · TARGET '+snapshot.inquiry.targetDropDate;}});
