import {loadInquiry,saveInquiry} from './inquiry.js';
...
async function save(kind){
  if(busy||!snapshot)return;capture();const expectedRevision=snapshot.revision;
  busy=true;error='';conflict=false;render();
  try{
    const result=await withAdmin(adminKey=>kind==='schedule'?saveSchedule({...scheduleDraft,adminKey,expectedRevision}):saveInquiry({scope,profile:drafts[scope],clear:kind==='clear',adminKey,expectedRevision}));
    ...
    if(kind==='schedule')adopt(result,true);
    ...
    onSaved(result);notify(kind==='schedule'?'Schedule saved · inquiry date synchronized':'Tuner settings synced to publisher');
...
export function setupTuner(options){
  ...
  dialog().addEventListener('click',e=>{const b=e.target.closest('button');if(!b||busy)return;
    ...
    if(b.hasAttribute('data-tuner-schedule'))save('schedule');
  });
}
