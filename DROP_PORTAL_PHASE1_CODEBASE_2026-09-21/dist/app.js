...
window.addEventListener('previewchange',updatePlayer);window.addEventListener('popstate',()=>{stopProviderPlayback();editing=false;render()});window.addEventListener('pagehide',()=>write('interactions',interactions));for(const d of document.querySelectorAll('dialog'))d.addEventListener('click',e=>{if(e.target===d&&e.clientX<d.getBoundingClientRect().left)d.close()});$('#detail').addEventListener('close',()=>detailId=null);render();Promise.all([loadWeeklyFeed(),loadSchedule()]).then(([feedOk])=>{render();if(feedOk)toast('Weekly DROP feed synced · schedule armed')});
