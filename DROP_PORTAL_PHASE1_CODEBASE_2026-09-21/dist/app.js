...
const BUILD_ID='P2.7.1-regression-closeout-2026-09-23';window.__DROP_PORTAL_BUILD__=BUILD_ID;
...
function providerShell(item,embedded,slot,extraClass=''){
  ...
  return `<div class="track-preview provider-slot ${active?'is-provider-live':'is-provider-idle'} ${extraClass}" data-provider-slot="${esc(slot)}"><span>OFFICIAL TRACK PREVIEW · ${esc(embedded.provider)}</span>${active?`<div class="provider-frame-wrap is-provider-loading"><iframe data-provider-player data-provider-slot="${esc(slot)}" src="${esc(embedded.url)}" title="Play ${esc(item.title)} by ${esc(item.artistName)}" loading="eager" allow="autoplay"></iframe><i class="provider-load-progress" aria-hidden="true"></i><i class="provider-embed-shield" aria-hidden="true"><b></b></i></div><button class="provider-unload" data-provider-unload="${esc(slot)}" aria-label="Stop and unload ${esc(item.title)} preview">STOP / UNLOAD PLAYER</button>`:`<button class="provider-activate" data-provider-activate="${esc(slot)}" aria-label="Load ${esc(item.title)} Bandcamp player"><b>▶</b><span>LOAD BANDCAMP PLAYER<small>One provider player at a time</small></span></button>`}</div>`;
}
...
function initHero(){
  ...
  function draw(t){
    ...
    for(let j=0;j<rows;j++)for(let i=0;i<cols;i++){
      ...
      ctx.fillStyle=`rgba(76,222,244,${Math.min(.95,a)})`;ctx.fillText(char,i*cell+3,j*row+4);
    }
  }
  ...
  heroCleanup=()=>{intersection.disconnect();document.removeEventListener('visibilitychange',sync);reduced.removeEventListener('change',sync)};sync();
}
...
function activeProfile(){return getFeedProfile(drops[0]?.id)||(profiles.weekly?.week===drops[0]?.id?profiles.weekly:profiles.base)}function act(id,key){interactions[id]??={};interactions[id][key]=!interactions[id][key];interactions[id].updatedAt=Date.now();persist('interactions',interactions);render();if(detailId)detail(detailId,false);toast(key==='hidden'?'Track hidden · restore from this drop’s footer':key==='saved'?(interactions[id].saved?'Saved to your crate':'Removed from saved'):(interactions[id].heard?'Marked as heard':'Marked as unheard'))}
...
function auditionControl(t,slot){const direct=directPreview(t),embedded=embedPreview(t);if(direct)return playButton(t);if(embedded?.provider==='BANDCAMP')return providerShell(t,embedded,slot);return`<span class="preview-unavailable">${t.releaseDate>new Date().toISOString().slice(0,10)?`PREVIEW UNLOCKS ${esc(formatReleaseDate(t.releaseDate))}`:'PROVIDER PREVIEW UNAVAILABLE'}</span>`}
...
function destinationActions(t,includeDetails=true,includePreview=includeDetails,slot='track:'+t.id){
...
function module(m,ts){let title='',body='',cls='',sub='';if(m.id==='start'){title='<span class="cyan">↗</span> &nbsp; Start here';sub='YOUR FIRST THREE';cls='priority';body=`<div class="panel-body">${ts.slice(0,3).map(priority).join('')||'<p>No visible tracks. Restore hidden tracks below.</p>'}</div>`}else if(m.id==='signal'){title='Weekly signal';sub='THE SHAPE OF THIS DROP';cls='signal-panel';const p=activeProfile();let counts=lanes.map((_,i)=>ts.filter(t=>t.lane===i).length);body=`<div class="signal"><div class="stats"><div><strong>${String(ts.length).padStart(2,'0')}</strong><span>TRACKS SELECTED</span></div><div><strong>${String(feedState.source==='github'?mixes.length:(p.mixes?1:0)).padStart(2,'0')}</strong><span>DJ MIXES</span></div></div><div class="distribution">${counts.map(c=>`<span style="flex:${c}"></span>`).join('')}</div><div class="lane-key">${counts.slice(0,3).map((c,i)=>`<span>${['■','□','▧'][i]} ${['Future','Deep','Jungle'][i]} ${c}</span>`).join('')}</div>${meter('Discovery depth',p.depth,p.depth>65?'Deep dig':'Familiar ground')}${meter('Experimental bias',p.experimental)}${meter('Floor ↔ headphones',p.floor,p.floor+'% headphones')}<p class="signal-foot">${getFeedProfile(drops[0]?.id)?`Published profile snapshot · ${getFeedProfile(drops[0]?.id).source==='weeklyOverride'?'This week override':'Base profile'}`:`Your local profile · ${profiles.weekly?.week===drops[0].id?'This week override':'Base preferences'}`}</p></div>`}else if(m.id==='mix'){if(!activeProfile().mixes||!mixes.length)return '';title='DJ mix crate';sub=String(mixes.length).padStart(2,'0')+' SESSION'+(mixes.length===1?'':'S');const mx=mixes[0],embedded=embedPreview(mx);body=`<div class="mix-stack"><div class="mix"><div class="mix-icon">◉</div><div class="mix-copy"><span class="artist">${esc(mx.artistName)}</span><strong>${esc(mx.title)}</strong><span class="track-meta">${esc(feedState.source==='github'?(mx.reason||'Published weekly mix'):'Demo entry · listening link not supplied')}</span>${destinationActions(mx,false)}</div>${buttons(mx)}</div>${embedded?providerShell(mx,embedded,'mix:'+mx.id,'mix-provider'):''}</div>`}else{let i=Number(m.id.slice(-1));let list=ts.filter(t=>t.lane===i);if(!list.length)return '';title=`<span class="marker m${i}"></span>${lanes[i]}`;sub=String(list.length).padStart(2,'0')+' TRACKS';body=`<div class="bin-grid">${list.map(card).join('')}</div>`}const sizes=m.id==='signal'?['compact','standard']:m.id==='start'||m.id==='mix'?['standard','wide']:['wide'];return `<section class="panel ${m.size} ${cls}" data-module="${m.id}">${editing?`<div class="module-controls"><button draggable="true" data-drag="${m.id}" aria-label="Drag ${m.id} module">⠿ Move</button><button data-move="-1" data-module-id="${m.id}" aria-label="Move ${m.id} up">↑</button><button data-move="1" data-module-id="${m.id}" aria-label="Move ${m.id} down">↓</button><label>Size <select data-size="${m.id}">${sizes.map(s=>`<option ${s===m.size?'selected':''}>${s}</option>`).join('')}</select></label></div>`:''}<div class="panel-head"><h2>${title}</h2><small>${sub}</small></div>${body}</section>`}
function header(title,sub,drop){
  const live=feedState.source==='github',test=live&&drop?.id?.startsWith('TEST-');
  const schedule=scheduleState.data,next=schedule?formatNextDrop(schedule):'SCHEDULE SYNCING';
...
function hourLabel(value){const hour=Number(String(value).slice(0,2));return `${hour%12||12}:00 ${hour<12?'AM':'PM'}`}
function scheduleControlMarkup(){
  const schedule=scheduleState.data;
  const fields=nextDropFields(schedule);
  const mode=scheduleMode(schedule);
  const hours=Array.from({length:24},(_,h)=>String(h).padStart(2,'0')+':00');
  const label=schedule?formatNextDrop(schedule):'SCHEDULE UNAVAILABLE';
  const stateLabel=schedule?.override?'SCHEDULE OVERRIDE':'WEEKLY DEFAULT';
  const localToday=new Date().toLocaleDateString('en-CA',{timeZone:'America/Chicago'});
  return `<section class="schedule-editor" aria-label="Drop schedule control"><div class="schedule-editor-head"><div><span>DROP SCHEDULE</span><small>Production publication control</small></div><em>DEFAULT · WED 7:00 PM CT</em></div><div class="schedule-row"><label><span>DAY</span><input id="schedule-date" type="date" min="${esc(localToday)}" value="${esc(fields.date)}"></label><label><span>TIME</span><select id="schedule-time">${hours.map(value=>`<option value="${value}" ${value===fields.time?'selected':''}>${hourLabel(value)}</option>`).join('')}</select></label><label><span>MODE</span><select id="schedule-mode"><option value="one-off" ${mode==='one-off'?'selected':''}>This drop only</option><option value="weekly-default" ${mode==='weekly-default'?'selected':''}>Make weekly default</option></select></label><button class="primary schedule-save" id="save-schedule" ${scheduleSaving?'disabled':''}>${scheduleSaving?'SAVING…':'SAVE'}</button></div><p class="schedule-armed">● NEXT DROP ARMED · ${esc(label)}</p><p class="schedule-help">${stateLabel} · America/Chicago · whole-hour scheduling</p></section>`
}
function openTuner(reset=true){
  if(reset)draft={base:{...profiles.base},weekly:{...(profiles.weekly?.week===drops[0].id?profiles.weekly:profiles.base)}};
  let p=draft[tunerScope];
...
function providerDockWave(){return `<div class="provider-dock-wave" aria-hidden="true">${providerDockBars.map(h=>`<i style="height:${h}px"></i>`).join('')}</div>`}
let dockId=null;function updatePlayer(){
  const dock=$('#dock'),t=player.current,providerItem=providerDockItem();
...
window.addEventListener('previewchange',updatePlayer);window.addEventListener('popstate',()=>{stopProviderPlayback();editing=false;render()});/* interaction state is written only on explicit user actions; avoid destructive pagehide rewrites */for(const d of document.querySelectorAll('dialog'))d.addEventListener('click',e=>{if(e.target===d&&e.clientX<d.getBoundingClientRect().left)d.close()});$('#detail').addEventListener('close',()=>detailId=null);render();Promise.all([loadWeeklyFeed(),loadSchedule()]).then(([feedOk])=>{render();if(feedOk)toast('Weekly DROP feed synced · schedule armed')});
